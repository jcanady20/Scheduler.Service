...
public class JobEngine
{
  ...
  private JobEngine(ILogger<JobEngine> logger, ScheduleContext scheduleContext, TaskManager taskManager)
  {
  ...
  public void Add(Job job, JobSchedule jobSchedule)
  {
    ...
    var je = new JobExecutioner(_logger, _db, job, jobSchedule, this.TaskManager);
    _activeJobs.Add(je);
  ...
  private void DeQueueAllJobs()
  {
    do
    {
      ...
      {
        if(exec.Status == Data.JobStatus.Canceled)
        {
          continue;
        }
        ExecuteJob(exec);
