...
//	AM/PM
Date.prototype.getMeridian = function () { var h = this.getHours(); return (h >= 12 ? "PM" : "AM"); };

//	Returns a 12 hour time with the meridian
//	1:05 PM
Date.prototype.getOClockTime = function () { var h = this.getHours(); var m = String("00" + this.getMinutes()).slice(-2); var timeString = ""; if (h > 12) { h = h - 12; } if (h === 0) { h = 12; } timeString += String("00" + h).slice(-2); timeString += ":" + m + " " + this.getMeridian(); return timeString; };

//	Returns Date String
//	01/01/1900 - 12/31/2099
Date.prototype.getDateString = function () { var m = this.getMonth(); var d = this.getDate(); var y = this.getFullYear(); return String("00" + (m + 1)).slice(-2) + "/" + String("00" + d).slice(-2) + "/" + y; };
...
window.app = (function (window, $, ko, _, Backbone, undefined) {
	var m_self = {};
	var m_debug = true;
	var m_router = null;
	...
	ViewModels.Schedule = function (data) {
		...
		_slf.type = ko.observable(enums.FrequencyType.Daily);
		_slf.interval = ko.observable(1);
		_slf.subdayType = ko.observable(enums.SubIntervalType.Hours);
		...
		_slf.subDayIntervalLabel = ko.pureComputed(function () {
			var x = parseInt(_slf.subdayType());
			if (x == enums.SubIntervalType.SpecificTime) {
				return "Time";
			} else if (x == enums.SubIntervalType.Hours) {
				return "Hour(s)";
			} else if (x == enums.SubIntervalType.Minutes) {
				return "Minute(s)";
			}
		}, _slf);
		...
		var getDescription = function () {
			var xType = parseInt(_slf.type());
			var msg = "";
			msg += "Occurs";

			if (xType === enums.FrequencyType.OneTimeOnly) {
				return msg += " one time only at " + _slf.startTime() + " on " + _slf.startDate();
			}
			else if (xType === enums.FrequencyType.Daily) {
				msg += " every ";
				if (_slf.interval() > 1) {
					msg += _slf.interval();
				}
			...
			else if (xType === enums.FrequencyType.Weekly) {
				msg += " " + _slf.recurrenceFactor() + " week(s)";
				msg += " on ";
				//	Get WeekDays
				msg += _.keys(_.pick(enums.WeeklyInterval, function (x) { return x & _slf.interval() }, this)).join(",");
			}
			else if (xType === enums.FrequencyType.Monthly) {
				msg += " every month(s) on day " + _slf.interval() + ".";
			}
			else if (xType === enums.FrequencyType.MonthlyRelative) {
			...
			else if (xType === enums.FrequencyType.OnStartup) {
				return msg += " when the schedule agent starts.";
			}

			//	Sub-day Details
			switch (parseInt(_slf.subdayType())) {
				case enums.SubIntervalType.SpecificTime:	//	At Specific Time
					msg += " at " + _slf.startTime() + ".";
					break;
				case enums.SubIntervalType.Minutes:	//	Minutes
					msg += " every " + _slf.subdayInterval() + " minutes(s) between " + _slf.startTime() + " and " + _slf.endTime() + ".";
					break;
				case enums.SubIntervalType.Hours:	//	Hours
		...
		var init = (function (data) {
			...
			var SetWeekDaySelections = function () {
				...
				var interval = parseInt(_slf.interval());
				var initialValues = _.values(_.pick(enums.WeeklyInterval, function (x) { return x & interval; }, this));
			...
			_slf.type.subscribe(function (newValue) {
				...
				if (x !== undefined && x !== NaN && x === enums.FrequencyType.OneTimeOnly) {
					_slf.subdayType(enums.SubIntervalType.SpecificTime);
				}
			}, this);
	...
	Views.JobCreate = Backbone.View.extend({
		...
		showDialog: function () {
			this.$el.find(".modal").modal({ backdrop: "static", keyboard: false, show: true });
		},
		OnClose: function () {
			this.remove();
		},
	...
	Views.JobStepEditor = Backbone.View.extend({
		...
		koViewModel: {
			...
			enabled: ko.observable(true),
			name: ko.observable(),
			subSystem: ko.observable(),
			command: ko.observable(),
			isVisShipped: ko.observable(false),
			...
			toJS: function () {
				...
				delete x["hasPreDefinedSubSystems"];
				delete x["selectedSubSystem"];
				delete x["isReadOnlyCommand"];
				return x;
			},
			init: function () {
				this.subSystems = m_subSystems.toJSON();
				return this;
			}
		},
		initialize: function () {
			_log("Initializing " + this.viewName);
			this.koViewModel.init();
			this.render();
		},
		render: function () {
			this.$el.empty();
